// The public issue trackers offered on `/support` - one per repository, since a bug in
// the iOS app isn't the web app's to fix. Shared with `SupportForm`, which falls back to
...
// configured. The form lives in the web app, so "the support form is broken" belongs on
